...
CAN DBC files encode signal layout with a `start_bit | length @ byte_order` triplet like `24|16@0+`. The `@0` vs `@1` flag selects Motorola (big-endian) or Intel (little-endian). In DBC bit numbering, each byte's bits are numbered 0 (LSB) to 7 (MSB), and `start_bit` is the position of the signal's **MSB** for Motorola, or its **LSB** for Intel.

For Motorola, the signal fills from the start bit toward **higher** byte numbers: the MSB sits at `start_bit`, then each following bit steps down within the byte (7 → 0); when a byte is exhausted, it continues at bit 7 of the next byte. A 16-bit signal at `start_bit=24` therefore occupies byte 3 bit 0 (MSB), then byte 4 bits 7 → 0, then byte 5 bits 7 → 1 (LSB) — the high byte is byte 3, the low byte is byte 5.

The `_signal_bit_positions()` function decomposes this into an explicit position list:

```python
# Motorola 16-bit signal at start_bit=24:
# Result: [(3, 0, 15), (4, 7, 14), (4, 6, 13), ..., (4, 0, 7), (5, 7, 6), ..., (5, 1, 0)]
#          byte 3 bit 0, MSB (shift 15) → byte 5 bit 1, LSB (shift 0)

positions = _signal_bit_positions(start_bit=24, length=16, byte_order="motorola")
assert positions[0]  == (3, 0, 15)   # MSB
assert positions[-1] == (5, 1, 0)    # LSB
```
